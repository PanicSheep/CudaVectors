...
template<typename> class host::DeviceVector;
...
public:
	...
	__host__ void assign(const host::DeviceVector<T>&, chronosity);

	__host__ void store(const std::vector<T>&);
	__host__ void store(const HostVector<T>&, chronosity);
	__host__ void store(const host::DeviceVector<T>&, chronosity);
...
template<typename T>
__host__ void HostVector<T>::assign(const host::DeviceVector<T>& src, chronosity chrono)
{
	assert(m_capacity >= src.size());
...
template<typename T>
__host__ void HostVector<T>::store(const host::DeviceVector<T>& src, chronosity chrono)
{
	if (m_capacity < src.size())
	{
		HostVector<T> new_vec{ src.size() };
